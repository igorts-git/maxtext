...
# The kernels an update is made of, in the order a step runs them. `lower()` and
# `compile_kernels()` are keyed by these, and so is the ahead-of-time entry point in
...
def _is_jax_dynamic(value: Any) -> bool:
  ...
  counts too: `lower()` drives the whole path on shapes alone, and an aval closed over as a
...
class MaxTextTrainingEngine(abstract_engine.AbstractTrainingEngine):
  ...
  def _compile_for_batch(self, dynamic_batch: Any, static_batch: dict[str, Any]) -> None:
    ...
    # The same three wrappers by name, which is what `lower()` traces: `compile()` overwrites
  ...
  def compile(
    ...
    """Triggers SPMD compilation of fwd_bwd and update steps.
  ...
  def compile_kernels(
    ...
    if compiler_options is None and getattr(self._config, "compile_xla_flags", ""):
      compiler_options = max_utils.parse_libtpu_flags_to_dict(self._config.compile_xla_flags)
    lowered = self.lower(dummy_data)
    with self._sharding_ctx():
  ...
  def lower(self, dummy_data: abstract_engine.TrainerPayload) -> dict[str, jax.stages.Lowered]:
    ...
    Routes through `_compile_for_batch`, the same method the live path calls on its first
    `fwd_bwd`, so the shapes and the shardings are the live ones by construction. The
    accumulating kernel is lowered even for a single-micro-batch run, where the live engine
    never traces it: omitting the kernel that holds the extra parameter-sized accumulator would
    understate the peak that decides whether a configuration fits.
    ...
    """
    if dummy_data is None:
      raise ValueError("lower() needs a dummy payload -- unlike compile(), it cannot defer to the first real batch.")
