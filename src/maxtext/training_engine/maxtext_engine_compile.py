...
class AbstractMaxTextEngine(maxtext_engine.MaxTextTrainingEngine):
  ...
  Enough to trace and lower every kernel, which is all `lower()` needs, while nothing is
  allocated and no checkpoint, tokenizer or network is touched. Nothing can be executed.
  """
  ...
  def _cannot_run(self, operation: str) -> RuntimeError:
    ...
    return RuntimeError(
        ...
        "exists to be traced and lowered: call `lower()`, or build a MaxTextTrainingEngine instead."
    )
