...
So these tests compare the HLO byte for byte, for all three kernels, against a live engine that
has actually stepped. `test_the_comparison_can_fail` guards the vacuous case.
...
class _LiveKernels:
  ...
  An executable keeps no handle on the lowering it came from, so reading a kernel's HLO means
  lowering it a second time -- through the engine's own jitted wrappers, from the avals of the
  arguments it was dispatched with. Avals rather than the arrays, because `_compiled_update`
  donates its state. That re-lowering is only sound if it reproduces the original, which
  `test_lowering_reproduces_the_kernels_training_ran` checks and the rest of the file rests on.
  """
  ...
  def lowered(self) -> dict[str, jax.stages.Lowered]:
    ...
    with self._engine._sharding_ctx():
      return {name: self._engine._jitted_kernels[name].lower(*calls[0]) for name, calls in self.calls.items()}
...
class EngineAotParityTest(parameterized.TestCase):
  ...
  def test_aot_compiles_the_same_hlo_the_trainer_runs(self, overrides):
    ...
    aot_lowered = maxtext_engine_compile.AbstractMaxTextEngine(cfg, mesh).lower(_abstract(_batch(cfg, 0)))

    self.assertEqual(sorted(live_lowered), sorted(aot_lowered))
    for name in live_lowered:
      # StableHLO with no normalization at all: nothing in it legitimately differs between two
      # lowerings of the same kernel.
      self.assertEqual(
          live_lowered[name].as_text(),
          aot_lowered[name].as_text(),
          f"{name}: the AOT lowering differs from the one training ran",
      )
      # And after optimization, which is what the cost and memory analyses are measured on.
      self.assertEqual(
  ...
  def test_the_comparison_can_fail(self):
    ...
    lowered = maxtext_engine_compile.AbstractMaxTextEngine(cfg, mesh).lower(_abstract(_batch(cfg, 0)))
    lowered_wider = maxtext_engine_compile.AbstractMaxTextEngine(wider, mesh).lower(_abstract(_batch(wider, 0)))
  ...
  def test_lowering_reproduces_the_kernels_training_ran(self):
    ...
    lowerings and none against training.
    """
    cfg, mesh = _config_and_mesh()
  ...
  def test_the_hlo_dump_filters_match_the_names_xla_gives_the_kernels(self):
    ...
    self.assertLen(lowered, len(maxtext_engine_compile.KERNEL_NAMES))
    for kernel in lowered.values():
      module = re.search(r"^module @(\S+)", kernel.as_text(), re.MULTILINE)
      self.assertIsNotNone(module, "the lowering has no module name to match against")
