...
class TrainTests(unittest.TestCase):
  ...
  def _llama4_losses(self, run_name, extra_args):
    """Trains a tiny Llama4 model for a few steps and returns its per-step losses."""
    with tempfile.TemporaryDirectory() as tmp_dir:
      metrics_file = os.path.join(tmp_dir, "metrics.txt")
      train_main(
          [
  ...
  def test_tpu_llama4_explicit_sharding_matches_auto(self):
    ...
    for name, parallelism_args in parallelism.items():
      with self.subTest(parallelism=name):
        auto_losses = self._llama4_losses(f"llama4_{name}_auto", parallelism_args + ["shard_mode=auto"])
        explicit_losses = self._llama4_losses(f"llama4_{name}_explicit", parallelism_args + ["shard_mode=explicit"])
  ...
  def test_tpu_llama4_zero1_gradient_accumulation(self):
    ...
    baseline = self._llama4_losses("llama4_ga", zero1_ga + ["shard_mode=auto", "shard_optimizer_over_data=False"])
    sharded = self._llama4_losses("llama4_ga_zero1", zero1_ga + ["shard_mode=explicit", "shard_optimizer_over_data=True"])
